...
def best_first_search(initial, goal):
    ...
    initial_state = PuzzleState(initial, heuristic(initial, goal))
    ...
    while not open_list.empty():
        ...
        for successor in generate_successors(current_matrix):
            ...
            if successor_tuple not in visited:
                ...
                successor_state = PuzzleState(successor, heuristic(successor, goal), current_state)
...
def main():
    ...
    print("Goal State:")
    printer(goal)
    if best_first_search(initial, goal):
